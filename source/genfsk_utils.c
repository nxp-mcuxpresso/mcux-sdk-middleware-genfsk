...
genfskStatus_t GENFSK_OverrideFrequency(uint32_t frequency)
{
    ...
    else
    {
        ...
        {
            (void)XCVR_OverrideTxFrequency(frequency, refOsc);
        }
#elif (NXP_RADIO_GEN == 350)
        (void)XCVR_OverrideFrequency(frequency);
#else
        (void)XCVR_OverrideTxFrequency(frequency);
#endif
    }
...
void GENFSK_SaveXcvrDcocDacTrimToFlash(xcvr_DcocDacTrim_t *xcvrDacTrim)
{
    ...
    FLib_MemCpy(&pHWParams->xcvrCal, xcvrDacTrim, (uint16_t)sizeof(xcvr_DcocDacTrim_t));
    OSA_EnableIRQGlobal();
...
bool_t GENFSK_RestoreXcvrDcocDacTrimFromFlash(xcvr_DcocDacTrim_t *xcvrDacTrim)
{
    ...
    status = FLib_MemCmpToVal(&pHWParams->.xcvrCal, 0xFF, (uint16_t)sizeof(xcvr_DcocDacTrim_t));

    if (status == FALSE)
    {
        /* XCVR Cal is valid, we can restore it */
        FLib_MemCpy(xcvrDacTrim, &pHWParams->xcvrCal, (uint16_t)sizeof(xcvr_DcocDacTrim_t));
    }
